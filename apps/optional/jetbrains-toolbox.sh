...
case "$(uname -s)" in
  Linux)
    ...
    # Toolbox ships as an AppImage, which needs FUSE. The package is libfuse2
    # up to Ubuntu 22.04 and libfuse2t64 from 24.04 on.
    ...
    # Copy the whole extracted tree: releases have shipped the binary both at
    # the top level and under bin/, and newer ones carry files beside it.
