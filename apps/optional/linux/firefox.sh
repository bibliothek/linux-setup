...
# install from Mozilla's APT repository instead. Written with tee (not tee -a)
# so re-running does not duplicate the source entry.
...
# Pin the Mozilla packages above the distro ones, otherwise apt keeps resolving
# firefox to the snap transitional package.
